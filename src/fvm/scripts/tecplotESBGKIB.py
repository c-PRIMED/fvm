...
def esbgkTecplotFile(meshes, macroFields,geomFields, mtype,filename ):
  #cell sites
  cellSites = []
  n=0
 ...
 # print "cellSites[", n, "].getCount = ", cellSites[n].getCount()
  ...
  ibType = []
  ibType.append( geomFields.ibType[cellSites[n]].asNumPyArray() )
  #face sites
  faceSites = []
  ...
  for i in range(0,ncell):
    if ( ibType[n][i]==-1 ):
      ...
      for node in range(0,nnodes_per_cell):
        nodeIndex[cellNodes[n](i,node)+1]=1
