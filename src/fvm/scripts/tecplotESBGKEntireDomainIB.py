...
def esbgkTecplotEntireDomain(nmesh, meshesLocal, meshesGlobal, mtype, macroFields,geomFields,filename ):
  #cell sites
  cellSites = []
  ...
  ibType=[]
  for n in range(0,nmesh):
      ibType.append(geomFields.ibType[cellSitesLocal[n]].asNumPyArray())
  ...
  for n in range(0,nmesh):
    ...
    ibTypeGlobal.append( zeros((selfCount,1), float) )
    ...
    #re-lable
    ibTypeFieldGlobal  = ibTypeGlobal[n]
    ibTypeFieldLocal   = ibType[n]
    ...
    for i in range(0,selfCount):
       ...
       ibTypeFieldGlobal[globalID] = ibTypeFieldLocal[i]
    ...
    MPI.COMM_WORLD.Allreduce(MPI.IN_PLACE,[ibTypeFieldGlobal, MPI.DOUBLE], op=MPI.SUM)
