...
def check_matrix(options):
    check_file     = os.path.join(options.outdir, 'matrix_mesh0.mat')
    os.system("more "+check_file)
    reference_file = os.path.join(options.golden, 'matrix_mesh0.mat')
    if numfile_compare.check_files(check_file, reference_file, 1.0e-8):
       print "convergence history files didn't match GOLDEN/* !!!!!!!!!!!!!"
       return  -1
      # break;
    print "matrix files are ok"
    return 0
...
def main():
    funcs = {
        ...
        'testThermalParallelJacobi.py':[check_convergence],
	      ...
	      'testThermalParallel.py':[check_matrix, check_rhs]
        }
    parser = OptionParser()
