...
def main():
	...
	'parallel_cylinder2D.py':[check_convergence],
	'parallel_sphere.py':[check_convergence]
        }
    parser = OptionParser()
    ...
    print "cwd = ", os.getcwd()
    mpi = 'mpirun -np %s %s %s  %s --type=%s' % (options.np, options.script, options.infile, options.infile2, options.type)
    print "MPI = ", mpi
    if options.xdmf:
        mpi += ' --xdmf'
