...
deformation =  structureFields.tractionX[mesh.getCells()].asNumPyArray()
fileName = fileBase + "deformation.dat"
    ...
    for fg in fgs:
        ...
        if fg.id==4:
            ...
            for i in range(0,nFaces):
                ...
                r = math.sqrt(x*x+y*y)
                theta = math.atan(y/x)
                term1 = ((a*a)/(r*r))*(0.5*math.sin(2.*theta)+math.sin(4.*theta))
                term2 = (3./2.)*(math.pow(a,4.)/math.pow(r,4.))*math.sin(4.*theta)
                
                sigmaXY = 10000*(-term1+term2)
                
                term1 = 1.0
                term2 = ((a*a)/(r*r))*((3./2.)*math.cos(2.*theta)+math.cos(4.*theta))
                term3 = (3./2.)*(math.pow(a,4.)/math.pow(r,4.))*math.cos(4.*theta)
            
                sigmaXX = 10000*(term1-term2+term3)

                term1 = ((a*a)/(r*r))*(0.5*math.cos(2.*theta)-math.cos(4.*theta))
                term2 = 1.5*(math.pow(a,4.)/math.pow(r,4.))*math.cos(4.*theta)

                sigmaYY = 10000.0*(-term1-term2)
                file.write(" %e " % xf[i][0])
                file.write(" %e " % xf[i][1])
                file.write(" %e " % theta)
                file.write(" %e " % deformation[faceCells(i,0)][0])
                file.write(" %e " % sigmaXX)
                file.write(" %e " % deformation[faceCells(i,0)][1])
                file.write(" %e " % sigmaXY)
                file.write(" %e " % deformation[faceCells(i,0)][2])
                file.write(" %e " % sigmaYY)
                file.write("\n")
file.close() 
...
xf =  geomFields.coordinate[cells].asNumPyArray()
...
rmsYY = 0.0
volumeSum = 0.0
for i in range(0,cells.getSelfCount()):
    x = xf[i][0]
    y = xf[i][1]
    r = math.sqrt(x*x+y*y)
    theta = math.atan(y/x)
...
#    deformation[i][1] = 10000*(-term1+term2)
    def1 = 10000*(-term1+term2)
    rmsXY = rmsXY + math.pow((deformation[i][1]-def1),2.0)*volume[i]
    deformation[i][1] = (abs(def1-deformation[i][1]))*100./10000. 
    
    term1 = 1.0
    term2 = ((a*a)/(r*r))*((3./2.)*math.cos(2.*theta)+math.cos(4.*theta))
    term3 = (3./2.)*(math.pow(a,4.)/math.pow(r,4.))*math.cos(4.*theta)
    
#    deformation[i][0] = 10000*(term1-term2+term3)
    def0 = 10000*(term1-term2+term3)
    rmsXX = rmsXX + math.pow((deformation[i][0]-def0),2.0)*volume[i]
    volumeSum = volumeSum + volume[i]
    deformation[i][0] = (abs(def0-deformation[i][0]))*100./10000.
    
    term1 = ((a*a)/(r*r))*(0.5*math.cos(2.*theta)-math.cos(4.*theta))
    term2 = 1.5*(math.pow(a,4.)/math.pow(r,4.))*math.cos(4.*theta)
    
#    deformation[i][2] = 10000.0*(-term1-term2)
    def2 = 10000.0*(-term1-term2)
    rmsYY = rmsYY + math.pow((deformation[i][2]-def2),2.0)*volume[i]
    deformation[i][2] = (abs(def2-deformation[i][2]))*100./10000.

rmsXX = math.sqrt(rmsXX/volumeSum)
rmsXY = math.sqrt(rmsXY/volumeSum)
rmsYY = math.sqrt(rmsYY/volumeSum)
print '\n rms error is = %f %f %f' % (rmsXX,rmsXY,rmsYY)
