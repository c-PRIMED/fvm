...
fileBase = None
numIterations = 100
fileBase = "/home/ba01/u119/das5/memosa/src/fvm/test/shankha/structure/circularHole/hole13"
...
#     print "shape( coords[", n, "] ) = ", shape( coords[n] )
    ...
    f = open("tecplot_hole13_dispError.dat","w")
...
traction =  structureFields.tractionX[mesh.getCells()].asNumPyArray()
a = 0.5
k = 3.-4.*nu
#k = (3.-nu)/(1.+nu)
mu = E/(2.*(1.+nu))
...
for i in range(0,cells.getSelfCount()):
    ...
    def0 = term*(term1+term2-term3)
    term1 = (1-k)*(a*a/r)
    term2 = r
    term3 = math.pow(a,4.0)/math.pow(r,3.0)
    def1 = term*(term1-term2-term3)*math.sin(2.*theta)
    def2 = 0.
    dmag = math.sqrt(def0*def0+def1*def1)
    rmsMag = rmsMag + math.pow((dmag-wmag),2.0)*volume[i]
    volumeSum = volumeSum + volume[i]
    deformation[i][0]=wmag
    deformation[i][1]=dmag
    traction[i][0] = (abs(dmag-wmag))/(dmag)*100.
    
rmsMag = 1.e6*math.sqrt(rmsMag/volumeSum)
print '\n rms error is = %f' % (rmsMag)
