...
def main():
    funcs = {
        ...
        'testThermalMultipleMeshes4.py'         : [check_convergence],
        'testThermalMultipleMeshes2.py'         : [check_convergence]
        }
    parser = OptionParser()
    ...
    mpi = ' %s %s --type=%s' % (options.script, options.infile, options.type);
    if options.xdmf:
        mpi += ' --xdmf'

    if os.system( mpi ):
        cleanup(-1)
    print "options.script = ", options.script
