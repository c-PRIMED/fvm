...
#     print "shape( coords[", n, "] ) = ", shape( coords[n] )	
  ...
  f.write("variables = \"x\", \"y\", \"z\", \"velX\", \"CellControidsX\", \"CellControidsY\", \"CellControidsZ\" \n")
  for n in range(0,nmesh):
	...
	if ( i % 5  == 4 ):
     ...
     #write velX
     for i in range(0,ncell):
        f.write( str(cellCentroids[n][i][0]) + "    ")
	if ( i % 5  == 4 ):
	    f.write("\n")
     f.write("\n")
	...
	if ( i % 5  == 4 ):
     ...
     #write velZ
     for i in range(0,ncell):
        f.write( str(cellCentroids[n][i][2]) + "    ")
	if ( i % 5  == 4 ):
	    f.write("\n")
     f.write("\n")
...
fname = "dumpMatrix" + str( MPI.COMM_WORLD.Get_rank() )
...
tSolver.relativeTolerance = 1e-4
tSolver.nMaxIterations = 20000
tSolver.maxCoarseLevels=0
tSolver.verbosity=3
