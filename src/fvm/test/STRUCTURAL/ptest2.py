...
def main():
	...
	'testStructureModelTransientFirstOrder.py':[check_convergence], 
        ...
        'testStructureElectricModel.py':[check_convergence]   
   }
    parser = OptionParser()
    parser.set_defaults(np=1,outdir='',type='tri')
    parser.add_option("--in1", dest='infile1', help="Input file 1 (required).")
    parser.add_option("--in2", dest='infile2', help="Input file 2 (required).")
    ...
    print "cwd = ", os.getcwd()
    mpi = 'mpirun -np %s %s %s %s --type=%s' % (options.np, options.script, options.infile1, options.infile2, options.type)
    print "MPI = ", mpi
    if options.xdmf:
        mpi += ' --xdmf'
