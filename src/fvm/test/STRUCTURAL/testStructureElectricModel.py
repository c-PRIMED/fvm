...
def setDirichletCommonDisplacement(dmodel,geomFields,meshes,structureFields):
    ...
    common0 = dmodel.getCommon(nodes0,nodes1).asNumPyArray()
    common1 = dmodel.getCommon(nodes1,nodes0).asNumPyArray()
    length = len(common0)

    def1 = geomFields.dirichletNodeDisplacement[nodes1].asNumPyArray()
    coord0N0 =  geomFields.coordinate[nodes0].asNumPyArray()
    coord0K1 =  geomFields.coordinateK1[nodes0].asNumPyArray()
    for i in range(0,length):
        id0 = common0[i]
        id1 = common1[i]
#        print '\n coord0 is %e %e %i' %(coord0N1[id0,0],coord0N1[id0,1],length)
        def1[id1] = coord0N0[id0] - coord0K1[id0]
...
mmmoptions.nNodeDisplacementSweeps = 5000
mmmoptions.absTolerance = 1e-13
